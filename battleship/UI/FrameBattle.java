...
public class FrameBattle implements ActionListener, KeyListener {
    ...
    JFrame frame = new JFrame("Burning Battle");
    ...
    private void setAffondato(Position p) {
        ...
        do {
            ...
            dim++;
                ...
                && posAttuale.getCoordY() <= 9 && !playerMap.WATER(posAttuale));

        deleteShip(dim, statPlayer);
    }

    private void setAffondato(Ship naveAffondata) {
        int dim = 0;
        for (int i = naveAffondata.getXstart(); i <= naveAffondata.getXEnd(); i++) {
            for (int j = naveAffondata.getYstart(); j <= naveAffondata.getYEnd(); j++) {
